...
let penultimateSlideItem = null;
let previousSlideItem = null;
let newSlideItems = null
...
function previousProduct() {
  // TODO: Complete this function
  clearInterval(sliderInterval);
  const currentSlideItem = newSlideItems;

  newSlideItems = previousSlideItem;
  previousSlideItem = penultimateSlideItem
  penultimateSlideItem = currentSlideItem
  showProduct(newSlideItems);
}

function randomProductItem() {
  return PRODUCTS[Math.floor(Math.random() * PRODUCTS.length)]
}

function randomlySelect() {
  penultimateSlideItem = previousSlideItem;
  previousSlideItem = newSlideItems;
  newSlideItems = PRODUCTS[Math.floor(Math.random() * PRODUCTS.length)];
  if (newSlideItems !== penultimateSlideItem && newSlideItems !== previousSlideItem) {
    showProduct(newSlideItems);
  } else {
    randomlySelect();
  }
...
newSlideItems = PRODUCTS[PRODUCTS.length - 1]
